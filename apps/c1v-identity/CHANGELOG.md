...
### Added
- **Enterprise UI Overhaul (v2)**
  - Light/dark mode toggle with improved contrast
  - Logo and account settings in header
  - Enhanced sidebar navigation (Projects, Organization, Connectors, Team Settings, Documentation, Configuration)
  - Projects page with project list, search, and "New Project" option
  - Project selection navigates to Golden Records Dashboard
  - Claude-style connectors interface with enterprise systems (Salesforce, HubSpot, Stripe, QuickBooks, NetSuite, CSV Upload)
  - Enhanced Data Connector workflow with PII encryption simulation
  - Conditional "Run Golden Records" button (enabled after 2 data sources)
  - Golden Records dashboard with error breakdown by severity
  - Single Match Resolution interface with detailed merge view
  - Dashboard Builder with templates and OpenAI SQL generation
  - Logs tab for query/event tracking
  - Reusable UI components module (`src/demo/components.py`)
  - OpenAI integration for natural language to SQL (`src/demo/openai_integration.py`)

### Changed
- **UI Navigation Structure**
  - Projects moved from Organization page to dedicated sidebar menu item
  - Projects page shows all projects with active project indicator
  - Clicking project navigates to Organization page → Golden Records tab
  - Organization page now focuses on dashboard tabs without project list
  - Default landing page changed to Projects page
  - Project selection now shows breadcrumbs (project > {projectName}) instead of "Working on" header
  - "Single Match" tab renamed to "Record Merge Errors" with error breakdown table
  - Added error summary metrics in Golden Records dashboard
  - Added "Solve Record Merge Errors" CTA button in Golden Records that guides users to Record Merge Errors tab
