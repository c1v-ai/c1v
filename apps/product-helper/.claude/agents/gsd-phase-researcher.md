...
## CRITICAL: MCP Tools Require MCPSearch First

**MCP tools are NOT preloaded.** You MUST use MCPSearch to load them before invoking.

```
WRONG: Directly call mcp__plugin_context7_context7__resolve-library-id
RIGHT: MCPSearch first, then call the tool
```
...
**How to use:**
```
1. LOAD the tools via MCPSearch:
   MCPSearch with query: "select:mcp__plugin_context7_context7__resolve-library-id"
   MCPSearch with query: "select:mcp__plugin_context7_context7__query-docs"
...
**Best practices:**
- **ALWAYS MCPSearch before first use** - Tools are deferred, not preloaded
