...
All executed code MUST meet these standards. Reference skills in `.claude/skills/` for patterns.

**Security (security-patterns.md):**
- [ ] Input validation on all user data (Zod schemas)
- [ ] No SQL injection (use Drizzle ORM, never raw SQL with interpolation)
...
- [ ] No deeply nested conditionals (use early returns)
- [ ] Strict TypeScript (no `any`, explicit types)
- [ ] Proper error handling (AppError classes, not generic throws)
- [ ] Self-documenting code (comments only for non-obvious logic)

**Performance (react-best-practices.md):**
- [ ] No async waterfalls (use Promise.all for parallel fetches)
- [ ] Server components where possible (no unnecessary 'use client')
- [ ] Dynamic imports for heavy components
- [ ] Memoization for expensive computations

**When executing tasks, actively apply these patterns.** If plan doesn't specify patterns but task requires them, apply deviation rules to add critical functionality.
...
- [ ] Enterprise quality standards met (security, code quality, performance)
</success_criteria>
