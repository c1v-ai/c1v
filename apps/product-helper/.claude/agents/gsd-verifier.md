...
<enterprise_quality_checks>

## Enterprise Quality Verification

In addition to functional verification, check for enterprise quality standards.

**Reference:** `.claude/skills/` for detailed patterns.

### Security Checks (security-patterns.md)

```bash
# Input validation missing
grep -rn "req\.(body|query|params)" --include="*.ts" | grep -v "safeParse\|schema\|validate"

# Raw SQL (potential injection)
grep -rn "sql\`.*\$\{" --include="*.ts"

# Hardcoded secrets
grep -rn "password\|secret\|apikey\|api_key" --include="*.ts" | grep -E "['\"].*['\"]"

# Missing auth checks in API routes
for route in $(find app/api -name "route.ts"); do
  if ! grep -q "auth\|session\|getUser" "$route"; then
    echo "NO_AUTH: $route"
  fi
done
```
...
# Functions longer than 30 lines (approximate)
awk '/^(export )?(async )?function|^const .* = (async )?\(/ {start=NR} /^}$/ {if(NR-start>30) print FILENAME":"start"-"NR}' *.ts

# Console.log in production code (not tests)
grep -rn "console\.(log|warn|error)" --include="*.ts" --include="*.tsx" | grep -v "__tests__\|test\|spec"
```

### Performance Checks (react-best-practices.md)

```bash
# Client components that could be server
for file in $(grep -l "'use client'" components/**/*.tsx 2>/dev/null); do
  # Check if it actually uses client features
  if ! grep -qE "useState|useEffect|onClick|onChange" "$file"; then
    echo "UNNECESSARY_CLIENT: $file"
  fi
done

# Missing dynamic imports for heavy dependencies
grep -rn "^import.*from 'mermaid\|recharts\|d3'" --include="*.tsx" | grep -v "dynamic("
```
...
|----------|----------|--------|
| Security vulnerability | BLOCKER | Must fix before phase passes |
| Missing auth check | BLOCKER | Must add authentication |
| `any` type usage | WARNING | Should fix, not blocking |
| Console.log in prod | WARNING | Should remove |
| Unnecessary client | INFO | Consider optimizing |

**Include quality issues in verification report under "Anti-Patterns Found" section.**
