...
- `agent: <type>` - Explicit agent override (optional)
- `subsystem: <name>` - Domain subsystem for agent routing
- `tags: [...]` - Tags for agent routing fallback
...
- **Agent type** (from routing logic below)
...
For each plan, determine the appropriate specialized agent using routing rules from `@./.claude/get-shit-done/references/agent-routing.md`:

1. **Explicit override** — If `agent:` field exists in frontmatter, use that agent
2. **Subsystem match** — Match `subsystem:` to agent routing table:
   | Subsystem | Agent |
   |-----------|-------|
   | `testing` | `qa-engineer` |
   | `security`, `auth` | `devops-engineer` |
   | `api` | `backend-architect` |
   | `database` | `database-engineer` |
   | `ui`, `accessibility`, `mobile` | `ui-ux-engineer` |
   | `chat` | `chat-engineer` |
   | `langchain` | `langchain-engineer` |
   ...
   | `ci-cd`, `performance` | `devops-engineer` |

3. **Tag match** — If no subsystem match, check `tags:` array for keywords (first match wins)
4. **Fallback** — Use `gsd-executor` if no match

Store selected agent with each plan in inventory.
</step>
...
1. **Describe what's being built (BEFORE spawning):**
   ...
   **{Plan ID}: {Plan Name}** → `{agent_type}` (if parallel)
   ...
   Note: `{agent_type}` comes from the routing step (e.g., `qa-engineer`, `devops-engineer`, etc.)
