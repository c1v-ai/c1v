...
kb_chunk_refs: []
legacy_snapshot: apps/product-helper/.planning/phases/13-Knowledge-banks-deepened/_legacy_2026-04-26/9-stacks-atlas/01-phase-docs/SOURCES.md
...
- **Decisions consumed by this phase:** see `decisions[]` in the engine.json keyed on `target_field` containing `sources` or by manual mapping in the story tree.
...
1. `searchKB` retrieves top-3 chunks scoped to `{module: 9, phase: sources}` (post-G8/G9 ingest).
...
- **Runtime retrieval:** `searchKB(query, top_k, { module: 9, phase: 'sources' })` over the `kb_chunks` table (`0011a_kb_chunks.sql`, ivfflat lists=100; HNSW upgrade gated on `>10k` rows).
