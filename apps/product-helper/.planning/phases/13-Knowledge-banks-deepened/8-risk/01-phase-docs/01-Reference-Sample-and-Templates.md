...
phase_slug: reference-sample-and-templates
module: 8
artifact_key: module_8/reference-sample-and-templates
...
kb_chunk_refs: []
legacy_snapshot: apps/product-helper/.planning/phases/13-Knowledge-banks-deepened/_legacy_2026-04-26/8-risk/01-phase-docs/01-Reference-Sample-and-Templates.md
...
- **Decisions consumed by this phase:** see `decisions[]` in the engine.json keyed on `target_field` containing `reference-sample-and-templates` or by manual mapping in the story tree.
...
1. `searchKB` retrieves top-3 chunks scoped to `{module: 8, phase: reference-sample-and-templates}` (post-G8/G9 ingest).
...
- **Runtime retrieval:** `searchKB(query, top_k, { module: 8, phase: 'reference-sample-and-templates' })` over the `kb_chunks` table (`0011a_kb_chunks.sql`, ivfflat lists=100; HNSW upgrade gated on `>10k` rows).
