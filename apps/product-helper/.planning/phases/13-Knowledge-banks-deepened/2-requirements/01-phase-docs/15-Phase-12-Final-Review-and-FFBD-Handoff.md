...
phase_slug: phase-12-final-review-and-ffbd-handoff
module: 2
artifact_key: module_2/phase-12-final-review-and-ffbd-handoff
...
kb_chunk_refs: []
legacy_snapshot: apps/product-helper/.planning/phases/13-Knowledge-banks-deepened/_legacy_2026-04-26/2-requirements/01-phase-docs/15-Phase-12-Final-Review-and-FFBD-Handoff.md
...
- **Decisions consumed by this phase:** see `decisions[]` in the engine.json keyed on `target_field` containing `phase-12-final-review-and-ffbd-handoff` or by manual mapping in the story tree.
...
1. `searchKB` retrieves top-3 chunks scoped to `{module: 2, phase: phase-12-final-review-and-ffbd-handoff}` (post-G8/G9 ingest).
...
- **Runtime retrieval:** `searchKB(query, top_k, { module: 2, phase: 'phase-12-final-review-and-ffbd-handoff' })` over the `kb_chunks` table (`0011a_kb_chunks.sql`, ivfflat lists=100; HNSW upgrade gated on `>10k` rows).
