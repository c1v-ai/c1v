...
must_haves:
  truths:
    - "All 5 content views verified rendering correctly with real data from explorer tree navigation"
    - "Nav-config hrefs verified matching actual route file paths with no broken links"
    - "Data shapes verified compatible between DB (JSONB) -> route page -> section component props"
  artifacts: []
  key_links:
    - from: "app/(dashboard)/projects/[id]/requirements/page.tsx"
      to: "components/projects/prd-overview.tsx"
      via: "React component import (pre-existing, verified)"
      pattern: "PRDOverview"
    - from: "components/project/nav-config.ts"
      to: "app/(dashboard)/projects/[id]/requirements/*/page.tsx"
      via: "href navigation (pre-existing, verified)"
...
Purpose: These components and routes already exist from Phase 4-02 and earlier. This plan is verification-only -- it confirms they work with actual extracted data and fixes any rendering issues discovered. No files are expected to be modified unless bugs are found.
...
<task type="auto">
  ...
  <action>
    ...
    **If bugs are found:** Fix them inline and document what was fixed. Add any fixed files to the SUMMARY.
...
<success_criteria>
- EXPL-03: PRD content visible in formatted accordion view (verified)
- EXPL-04: System Overview renders actors, boundaries, entities (verified)
- EXPL-05: Tech Stack shows technologies with rationale and alternatives (verified)
- EXPL-06: Infrastructure shows hosting, CI/CD, monitoring, environments (verified)
- EXPL-07: Architecture Diagram renders Mermaid diagram (verified)
- All views accessible via explorer tree navigation (verified)
</success_criteria>
