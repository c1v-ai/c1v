...
- Issue: 27+ `console.log` calls with `[STATE_DEBUG]` prefix remain in the LangGraph handler. These produce verbose per-request output including data shapes, completeness scores, and checkpoint details. Additionally, `isLangGraphEnabled()` at line 1200 logs every time it is called.
...
- Files: Pervasive across all server-side code. Highest density in `lib/db/setup.ts` (37 calls), `lib/langchain/graphs/intake-graph.ts` (12 calls), `app/api/chat/projects/[projectId]/langgraph-handler.ts` (39 calls).
...
- Issue: Project data fields (`actors`, `useCases`, `systemBoundaries`, `dataEntities`) are cast to `any` whenever read from or written to the database. This occurs in 14 files across `app/actions/`, `app/(dashboard)/projects/`, and test files.
- Files: `app/actions/conversations.ts` (12 occurrences), `app/actions/projects.ts` (1), `app/(dashboard)/projects/[id]/data/page.tsx` (4), `app/(dashboard)/projects/[id]/diagrams/page.tsx` (4), plus 6 page components that pass `project as any` to section components
- Impact: Type mismatches between DB schema JSONB columns and TypeScript types silently pass through. Runtime errors from malformed data are not caught at compile time. Section page components (`tech-stack/page.tsx`, `api-spec/page.tsx`, `infrastructure/page.tsx`, `schema/page.tsx`, `system-overview/page.tsx`, `architecture/page.tsx`, `user-stories/page.tsx`) all use `project as any` which masks type incompatibilities.
- Fix approach: Define proper Drizzle inference types for JSONB columns. Create a `ProjectWithData` type that matches the actual DB query result. Use Zod parsing at the boundary rather than `as any`.
...
**Deprecated Diagram Generator Functions Still in Use:**
- Issue: Two functions are marked `@deprecated` but are still called: `generateContextDiagram()` and a second context diagram generator function.
- Files: `lib/diagrams/generators.ts` (lines 110 and 444)
- Impact: Deprecated code accumulates. Callers may use the old API producing lower-quality output while the newer industry-standard functions exist.
- Fix approach: Migrate all callers to `generateIndustryStandardContextDiagram()`. Remove deprecated functions.

**Duplicate `getUser()` Calls in `getTeamForUser()`:**
- Issue: `getTeamForUser()` internally calls `getUser()`, which reads and verifies the session cookie and queries the database. Every API route that calls both `getUser()` and `getTeamForUser()` sequentially executes `getUser()` twice -- two cookie reads, two JWT verifications, two database queries.
- Files: `lib/db/queries.ts` (lines 7-37 for `getUser()`, lines 102-130 for `getTeamForUser()`)
- Impact: Every authenticated API request performs 2x user DB queries. 30 call sites across 11 files in `app/` follow this pattern.
- Fix approach: Have `getTeamForUser()` accept an optional `userId` parameter to skip redundant `getUser()` call. Or refactor to a single `getAuthContext()` function that returns `{ user, team }` in one flow.

**Legacy Chat Mode (564 lines of dead-path code):**
- Issue: The `app/api/chat/projects/[projectId]/route.ts` file contains a complete legacy chat implementation (lines 211-393) that only runs when `USE_LANGGRAPH` env var is unset or false. This includes a hand-built prompt with hardcoded completeness thresholds, manual diagram extraction, and `StreamingTextResponse`. The LangGraph mode is the primary path.
- Files: `app/api/chat/projects/[projectId]/route.ts` (lines 211-393)
- Impact: 180+ lines of legacy code that must be maintained alongside the LangGraph path. Two code paths for the same feature creates confusion.
- Fix approach: If LangGraph is the production default, remove the legacy path or extract to a separate file for fallback purposes only.
...
**Direct DOM Manipulation for Mermaid SVG Rendering:**
- Risk: Mermaid-generated SVG is injected directly into the DOM via direct DOM manipulation in the diagram viewer component. While the SVG comes from the `beautiful-mermaid` package (not user input), the diagram syntax originates from LLM output which could potentially contain malicious SVG with embedded scripts.
- Files: `components/diagrams/diagram-viewer.tsx` (line 169)
- Current mitigation: SVG is generated by Mermaid library from syntax, not raw user HTML.
- Recommendations: Sanitize SVG output before DOM injection using DOMPurify. Or use a React-based SVG renderer to avoid direct DOM manipulation.
...
**No Input Sanitization on User-Provided Content Stored in DB:**
- Risk: User messages from chat, project names, and vision text are stored directly without sanitization. While React auto-escapes on render, MCP tools and export functions serve this data without HTML escaping.
- Files: `app/api/chat/projects/[projectId]/route.ts`, `app/actions/projects.ts`
- Current mitigation: React auto-escaping for rendered content. Zod validation enforces length limits.
- Recommendations: Add input sanitization for stored text. Ensure MCP tool responses and markdown exports escape special characters.

## Performance Bottlenecks

**1,202-Line LangGraph Handler (God File):**
- Problem: `langgraph-handler.ts` is 1,202 lines containing the main handler, streaming handler, post-intake generation (6 parallel LLM agent calls), entity derivation logic, diagram extraction, project data persistence, and the feature flag check.
...
- Improvement path: Extract `triggerPostIntakeGeneration()` into its own module (e.g., `lib/langchain/post-intake/generator.ts`). Extract entity derivation into `lib/langchain/post-intake/entity-derivation.ts`. Extract diagram save logic.
...
- Problem: `getTeamForUser()` calls `getUser()` internally. Routes that call both `getUser()` AND `getTeamForUser()` make 2 cookie reads + 2 JWT verifications + 2 user DB queries per request. This pattern is used in 11 route handler files with 30 total call sites.
- Files: `lib/db/queries.ts`, all API routes and server actions
- Cause: `getTeamForUser()` was designed to be self-contained, but callers also need user info separately.
- Improvement path: Create a unified `getAuthContext()` function returning `{ user, team }` with a single cookie/JWT/DB flow. Or cache `getUser()` result within a request scope using React cache.
...
**3,432 Lines of Type Definitions Across 4 Files:**
- Problem: Type definitions are spread across `lib/langchain/schemas.ts` (921 lines), `lib/db/schema/v2-types.ts` (837 lines), `lib/types/mcp.ts` (952 lines), and `lib/langchain/graphs/types.ts` (722 lines). Some types are duplicated or overlapping.
- Files: Listed above
- Cause: Organic growth. Each feature added its own types without consolidation.
- Improvement path: Audit for duplicate types. Consider barrel exports from a single `lib/types/` directory. Use Drizzle's inferred types instead of maintaining parallel type definitions.

**No Error Boundaries in App Routes:**
- Problem: Zero `error.tsx` files exist in the entire `/app` directory. Only one `loading.tsx` file exists (`app/(dashboard)/dashboard/activity/loading.tsx`). Unhandled errors in any page component crash the entire page with Next.js default error UI.
- Files: `app/` directory (no `error.tsx` files)
- Cause: Error boundaries were never added.
- Improvement path: Add `error.tsx` at `app/(dashboard)/error.tsx` and `app/(login)/error.tsx` at minimum. Add `loading.tsx` to data-heavy pages like project detail views.
...
**Knowledge Bank Injection into Agent Prompts (Active Work):**
- Files: `lib/education/generator-kb.ts` (894 lines), `lib/langchain/agents/tech-stack-agent.ts`, `lib/langchain/agents/api-spec-agent.ts`, `lib/langchain/agents/infrastructure-agent.ts`, `lib/langchain/agents/schema-extraction-agent.ts`, `lib/langchain/agents/guidelines-agent.ts`
- Why fragile: Knowledge bank content is injected via string interpolation into PromptTemplate strings (`${getTechStackKnowledge()}`). The KB functions return multi-page text blocks that significantly increase prompt token count. If KB content grows beyond token limits, agents will silently truncate or fail. No token counting is done before injection.
- Safe modification: Monitor token usage per agent. Add a max-length check on KB content before injection. Keep KB content focused and concise.
- Test coverage: No tests verify that KB-injected prompts stay within token limits.
...
- Current capacity: `estimateTokenCount()` uses `Math.ceil(text.length / 4)` which is a rough GPT-era approximation.
- Limit: Claude tokenization differs from GPT. Conversation history limits and token budgets will be inaccurate.
- Scaling path: Use Anthropic's token counting API or `@anthropic-ai/tokenizer` for accurate counts.
...
**`beautiful-mermaid@0.1.3` (Very Early Version):**
- Risk: Version 0.1.3 of a niche package for Mermaid rendering. Low usage, unknown maintenance status.
- Impact: If this package is abandoned, Mermaid rendering would need migration. SVG output quality depends on this package.
- Migration plan: Monitor package health. Fallback to direct `mermaid` package rendering if needed (already a dependency at 11.12.2).
...
**No Error Boundaries:**
- Problem: Zero `error.tsx` files exist in the app. Unhandled errors crash pages with Next.js default error UI with no recovery mechanism.
- Blocks: Graceful degradation for users when individual page components fail.
...
**No Tests for Knowledge Bank Token Budgets:**
- What's not tested: Whether KB content injected into agent prompts stays within Anthropic's context window limits.
- Files: `lib/education/generator-kb.ts` (894 lines of content)
- Risk: As KB content grows, agent calls may exceed token limits and silently fail or produce truncated output.
- Priority: Medium -- becomes higher as more KB content is added.
