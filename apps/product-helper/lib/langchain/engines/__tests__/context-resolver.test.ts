...
describe('ContextResolver — cross-module phase decisions', () => {
  ...
  it('M8: resolves fmea-residual severity via signals fallback when slug is outside MODULE_SLUGS enum', async () => {
    // module-8-risk is NOT in the engine.MODULE_SLUGS enum (which is
    // module-1..module-8). parseModuleRefString rejects the source string,
    // so the route has no artifact reference and the input falls through
    // to the signals bag — proving the resolver still projects an M8 phase
    // decision when the slug shape doesn't fit the enum.
    const decision: DecisionRef = {
