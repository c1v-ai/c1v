...
Agent({
  ...
  deliverables: [
    ...
    "**Day-0 baseline-capture (EC-V21-E.13 prereq):** if `plans/v21-outputs/observability/sentry-baseline-2026-04-25.json` does NOT exist on disk, write it by scraping the v2.1 7-day rolling Sentry window for `synthesis_metrics_total{module=\"m2\",impl=\"llm-only\"}`. File shape: `{ baseline_window: '2026-04-19 to 2026-04-26', counter: 'synthesis_metrics_total', label: 'module=m2,impl=llm-only', total_calls: <int>, projects_observed: <int>, captured_at: <iso8601>, captured_by: 'engine-core' }`. If file already exists, verify shape + skip rewrite.",
...
Agent({
  ...
  deliverables: [
    ...
    "5 schema extensions in `apps/product-helper/lib/langchain/schemas/` per master plan v2.1 line 475 — exact extensions surfaced during the rewrite. Likely candidates: `module-2/nfr-engine-extension.ts`, `module-6/qfd-engine-extension.ts`, `module-8/fmea-engine-extension.ts`, plus 2 cross-cutting. (Coordinate with TC1 if shape overlap with Wave C schemas — overlap is OK; double-authoring is NOT.)",
  ...
  guardrails: [
    ...
    "Schema extensions coordinate with TC1's `crawley-schemas` agent — if a Crawley schema already covers the shape, EXTEND in place rather than adding a v2.2 sibling. tsc-green is the integration gate.",
...
Agent({
  ...
  required_reading: ["plans/wave-e-day-0-inventory.md"],
  deliverables: [
    "apps/product-helper/components/synthesis/why-this-value-button.tsx — small button placed next to every auto-filled value in the synthesis viewer (NFRs, constants, decision-network choices, QFD weights, FMEA severities). Click → opens side-panel.",
...
Agent({
  ...
  subagent_type: "langchain-engineer",
  team: "c1v-kb-runtime-engine",
  goal: "Swap `GENERATE_nfr` + `GENERATE_constants` graph-node internals from v2.1's LLM-only agents to `nfrEngineInterpreter.evaluate(...)` via the **DI pattern locked in TE1's `nfr_node_swap_mechanism_LOCKED`** (`intake-graph.ts` accepts `nfrImpl?: 'llm' | 'engine'` arg with default 'engine' post-swap). NOT env-var, NOT fixture-override — agent does NOT improvise an alternative. Verify ≥60% LLM call rate drop on M2 against the baseline at `plans/v21-outputs/observability/sentry-baseline-2026-04-25.json` (written by `engine-core` Day-0 prereq). Both `intake-graph.ta1-integration.test.ts` describe blocks (one per impl: llm-only and engine-first) must pass with new internals. EC-V21-E.12 + EC-V21-E.13 gates. **LAST deliverable agent — runs after all 9 above.**",
  inline_skills: ["langchain-patterns", "claude-api", "code-quality", "testing-strategies"],
  required_reading: ["plans/wave-e-day-0-inventory.md", "plans/v21-outputs/observability/sentry-baseline-2026-04-25.json"],
  deliverables: [
  ...
  guardrails: [
    "HARD-DEP on ALL other 9 deliverable agents' tags (engine-context, audit-writer, engine-fail-closed, engine-gap-fill, engine-pgvector, engine-stories, kb-rewrite, provenance-ui).",
    ...
    "Baseline JSON path is `plans/v21-outputs/observability/sentry-baseline-2026-04-25.json` per the Day-0 prereq — DO NOT change.",
...
Agent({
  ...
  guardrails: [
    "Depend on engine-prod-swap (block on `te1-prod-swap-complete` tag) AND all 9 other deliverable agents (block on agent names).",
    "Non-fix verifier — log failures, capture evidence, surface to coordinator. Do NOT auto-fix any deliverable.",
    "All 14 EC sub-points must be green to tag. Partial green = NO TAG; surface failed EC(s) to coordinator with evidence.",
