...
Agent({
  ...
  deliverables: [
    ...
    "plans/v211-outputs/th1/e2e-evidence.md — capture: (a) test run video/screenshot link, (b) network HAR or excerpt showing the POST + status polls, (c) project_artifacts row-count timeline (0 → 7 pending → N ready), (d) total wall-time for the click-through.",
  ...
  guardrails: [
    ...
    "Test MUST NOT depend on a specific count of 'ready' artifacts — full synthesis completion is not the gate. The gate is: POST fires + at least 1 transition pending → ready (proves the wire is alive end-to-end). Failure scenarios (timeout, sidecar down) are TB1's coverage.",
